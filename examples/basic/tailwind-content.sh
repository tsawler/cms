#!/bin/sh
# CMS content-CSS build for Tailwind v4, wired via CMS_TAILWIND_COMMAND
# (see .env). The v4 CLI has no --content flag — it reads sources from
# the input CSS — so this stages a scratch input that @sources only the
