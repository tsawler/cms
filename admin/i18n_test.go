...
func TestTemplatesRenderInFrench(t *testing.T) {
	data := templateData{
		...
		Users: []userRow{
			{User: auth.User{ID: 2, Name: "A", Email: "a@example.com", Role: auth.RoleEditor, Active: true}, CanManage: true},
			{User: auth.User{ID: 3, Name: "B", Email: "b@example.com", Role: auth.RoleSuperadmin, Active: true}},
		},
